...
class TurnoverRanking(Base):
    ...
    # 排名與周轉率
    turnover_rank = Column(Integer, nullable=False)  # 周轉率排名 1-50
    turnover_rate = Column(Float, nullable=False)  # 周轉率 %

    # 價格資料
    close_price = Column(Float)
    prev_close = Column(Float)
    change_percent = Column(Float)  # 漲幅 %
    amplitude = Column(Float)  # 振幅 %

    # 成交資料
    volume = Column(Integer)  # 成交量(張)
    float_shares = Column(Float)  # 流通股數(萬股)
    volume_ratio = Column(Float)  # 量比

    # 漲停相關
    is_limit_up = Column(Boolean, default=False)  # 是否漲停
    limit_up_type = Column(String(20))  # 漲停類型: 一字板/秒板/盤中/尾盤
    seal_volume = Column(Integer)  # 封單量(張)
    seal_amount = Column(Float)  # 封單金額(萬元)
    open_count = Column(Integer)  # 開板次數
    first_limit_time = Column(String(10))  # 首次漲停時間

    # 其他指標
    consecutive_up_days = Column(Integer)  # 連續上漲天數
...
class FloatShares(Base):
    ...
    # 股本資料 (單位: 萬股)
    total_shares = Column(Float)  # 總股數
    float_shares = Column(Float)  # 流通股數
...
class TurnoverTrack(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    symbol = Column(String(10), nullable=False, index=True)
    trigger_date = Column(Date, nullable=False)  # 觸發日期
    turnover_rank = Column(Integer)  # 觸發時的周轉率排名
    trigger_price = Column(Float)  # 觸發價格

    # 追蹤結果
    day1_change = Column(Float)  # 隔日漲跌幅
    day1_limit_up = Column(Boolean)  # 隔日是否繼續漲停
    day3_change = Column(Float)  # 3日後漲跌幅
    day5_change = Column(Float)  # 5日後漲跌幅
    day7_change = Column(Float)  # 7日後漲跌幅
