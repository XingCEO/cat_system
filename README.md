...
### v1.1.0 (2026-02-01) - UI/UX 全面優化

#### 🎨 設計系統升級
- 採用 Fintech 專業設計系統（glassmorphism、glow effects）
- 統一所有頁面標題樣式（icon + 標題格式）
- 優化 shadcn/ui 元件視覺效果
- 新增 premium 視覺效果（hover glow、backdrop-blur）
- 改善深色模式對比度與可讀性

#### 🖨️ K 線圖列印功能
- **專業雙欄排版**：左側圖表區 (200mm) + 右側資訊面板 (83mm)
- **動態填滿**：根據 `chartWidth / barSpacing` 計算 K 線數量，不拉伸變形
- **鎖定列印**：支援 crosshair 鎖定，列印截止到指定日期
- **資訊面板**：當日價格、均線數值 (MA5-MA120)、圖例說明、資料範圍
- **A4 橫向**：單頁輸出，專業金融報表風格

#### 🆕 新增元件
- `ErrorBoundary.tsx` - React 錯誤邊界，優雅處理崩潰
- `NotFoundPage.tsx` - 404 頁面
- `RealtimeMonitorPage.tsx` - 即時監控頁面

#### 🔧 後端重構
- 模組化分析器架構 (`backend/services/analyzers/`)
- 新增 CI/CD workflow (`.github/workflows/ci.yml`)
- 新增中間件支援（認證、請求日誌）

#### 📦 Bundle 優化
- 程式碼分割：vendor-charts、vendor-react、vendor-data 分離
- CSS 整合：單一 47KB CSS 檔案
- 建置時間：2.23 秒

### v1.0.1 (2026-02-01) - 安全性修復
- 修復路徑穿越漏洞
- 收緊 CORS 配置
- CSV 匯出引號跳脫修復
- React Query 狀態更新規範化
